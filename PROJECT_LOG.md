...
- **렌더 경로 신설**: `ProjectDetailPage.js`에 `section.diagram` → 컴포넌트 레지스트리 매핑. `projects.js` sections에 `diagram` 키만 추가하면 확장 가능.
- **검증**: 테스트 11개 통과, 라이트/다크·언어·모바일 확인 후 배포, 라이브 재검증.
...
6.  **확인 필요** — Contact 이력서 링크만 새 탭 열기, 통일 여부 검토 (Hero/Navbar는 강제 다운로드).
